...
# Create missing .so.10.0 library links after the Cuda has been updated to the version 10.1
## Missing links are required after the Tensorflow PIP-installation
## This is a bug in the Tensorflow PIP-installation
for file in /usr/lib64/libcu*.so.10; do echo cp -P "$file" "$(echo $file | sed 's/so.10/so.10.0/g')"; done | /bin/bash
for file in /usr/local/cuda-10.1/targets/x86_64-linux/lib/*.so.10; do echo cp -P "$file" "$(echo $file | sed 's/so.10/so.10.0/g')"; done | /bin/bash
for file in /usr/local/cuda-10.1/targets/x86_64-linux/lib/*.so.10.1; do echo cp -P "$file" "$(echo $file | sed 's/so.10.1/so.10.0/g')"; done | /bin/bash

# Info: Verify .so.10 links
# ls -la /usr/lib64/libcu*.so.10
# ls -la /usr/local/cuda-10.1/targets/x86_64-linux/lib/*.so.10

# Info: Verify .so.10.0 links
# ls -la /usr/lib64/libcu*.so.10.0
# ls -la /usr/local/cuda-10.1/targets/x86_64-linux/lib/*.so.10.0
...
cd /root
wget http://172.28.0.253/centos7_x64_everything_1804/nvidia/libcudnn7-7.5.0.56-1.cuda10.1.x86_64.rpm
rpm -Uvh libcudnn7-7.5.0.56-1.cuda10.1.x86_64.rpm
...
cd /root
wget http://172.28.0.253/centos7_x64_everything_1804/nvidia/libcudnn7-devel-7.5.0.56-1.cuda10.1.x86_64.rpm
rpm -Uvh libcudnn7-devel-7.5.0.56-1.cuda10.1.x86_64.rpm
...
cd /root
wget http://172.28.0.253/centos7_x64_everything_1804/nvidia/libcudnn7-doc-7.5.0.56-1.cuda10.1.x86_64.rpm
rpm -Uvh libcudnn7-doc-7.5.0.56-1.cuda10.1.x86_64.rpm
...
#pip install --ignore-installed tf-nightly-gpu
